...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ...
      {...props}
    >
      {/* Required for accessibility; visually hidden so no visual change */}
      <VisuallyHidden>
        <SheetPrimitive.Title>Sheet dialog</SheetPrimitive.Title>
      </VisuallyHidden>

      <SheetPrimitive.Close className="absolute right-4 top-4 opacity-70 hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-offset-2">
        <X className="h-4 w-4" />
        <span className="sr-only">Close</span>
      </SheetPrimitive.Close>
